...
void loop() {
  ...
  if (peripheral) {
     ...
     */
    if (peripheral.localName() == "00101594") {
      // stop scanning
      BLE.stopScan();
...
void monitorSensorTagButtons(BLEDevice peripheral)
{
  ...
  Serial.println("Discovering attributes of service Auth Bluetooth Device ...");
  if (peripheral.discoverAttributesByService("Auth Bluetooth Device")) {
  ...
  BLECharacteristic simpleKeyCharacteristic = peripheral.characteristic("Bluetooth MAC Address String");
